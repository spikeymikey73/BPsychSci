...
:::{.alert .alert-info}
#### <i class="fas fa-info-circle"></i> Understand the difference between population and sample
...
:::{.alert .alert-info}
##### <i class="fas fa-info-circle"></i> Distinguish between parameters and statistics
...
- **Statistics are used to estimate values that exist in the population.**
:::

### LO3

:::{.alert .alert-info}
#### <i class="fas fa-info-circle"></i> Distinguish between descriptive and inferential statistics
...
:::{.alert .alert-info}
#### <i class="fas fa-info-circle"></i> Distinguish between discrete and continuous variables
...
- **Continuous**:
  - Contain many different values - Weight (40kg- 140kg), age (0-100 years)
...
:::{.alert .alert-info}
#### <i class="fas fa-info-circle"></i> Explain the difference between
...
:::{.alert .alert-info}
#### <i class="fas fa-info-circle"></i> Distinguish between independent and dependent variables
...
:::{.alert .alert-info}
##### <i class="fas fa-info-circle"></i> Understand the different types of measurement scales (nominal, ordinal, interval and ratio)
...
- **Ordinal:** Ordinal scales consist of a set of categories with different names AND are organised into an ordered sequence
...
:::{.alert .alert-info}
#### <i class="fas fa-info-circle"></i> Understand how to construct frequency distributions and histograms

- **Frequency distributions** report how often each score was recorded in a given data set.
- When presented in table form, a frequency distribution lists all values for a variable along with the frequency of each.
...
- The frequency table above helps to summarise the collected data, allowing us to easily see the number of participants recording each score.
- We can also gather from this table the highest concentration of results towards the centre of the table (those who answered 4 or 5).
- The **histogram** is a graphical representation of the data in the frequency table.
- A larger data set may be most relevant when the results are grouped together (known as a **bin**), while smaller data sets can simply be plotted as collected.
- Grouping data in bins assists in reducing the number of 'random' results and to assist in identifying trends.
:::

### LO9

:::{.alert .alert-info}
#### <i class="fas fa-info-circle"></i> Understand positive skew, negative skew and kurtosis
...
## Week 2:  Central Tendency, Variability and z-scores {.well .tabset .tabset-fade .tabset-pills}

### LO1

:::{.alert .alert-info}

#### <i class="fas fa-info-circle"></i> Statistical Notation
...
$$
\Sigma X = (X_1 + X_2 + X_3 + X_4 + X_5) = 197 \\
\Sigma X^2 = (X_1^2 + X_2^2 + X_3^2 + X_4^2 + X_5^2) = 8247 \\
(\Sigma X)^2 = (X_1 + X_2 + X_3 + X_4 + X_5)^2 = 38809
$$
...
:::{.alert .alert-info}
#### <i class="fas fa-info-circle"></i> Measures of Central Tendency
...
- Once data is collected, research psychologists would usually be interested in examining the <abbr title="the mid-point of a frequency distribution">central tendency</abbr> of this data. 
  - **Mode**: The most frequently occurring value and thus represented by the highest bar in a histogram.
  - **Median**: The score that corresponds to the point at or below which 50% of the scores fall when the data are arranged in numerical order.
...
#### <i class="fas fa-info-circle"></i> Relative Pros & Cons
...
:::{.alert .alert-info}
#### <i class="fas fa-info-circle"></i> Variability
...
- Average Deviation
  - The degree to which scores vary from the mean, when calculating the mean and then compute how much each score deviates from this average value.
  - We then average these deviations to provide a single measure of variability.
  - However, this would not work as some scores will be above, and some below the mean, resulting in positive and negative deviations (which will serve to balance each other out and sum to zero).
- Mean Absolute Deviation (m.a.d.)
  - The mean absolute deviation (m.a.d.) overcomes this problem by using the absolute values of the deviations.
  - This is helpful as we are not interested in whether scores are above or below the mean, simply the extent to which they vary from the mean.
  - The m.a.d. is the sum of the absolute deviations divided by N to provide an average deviation.
- Variance ($S^2$)
  - This statistic uses squared deviations to deal with the problem of positive/negative differences from the mean.
  - A negative value, once squared, becomes positive.
  - So we calculate the sum of the squared deviations from the mean (or sums of squares, $SS$).
...
- It provides a measure of the average deviation from the mean and is the most commonly reported statistic of variability.
- Once the $s^2$ has been calculated, it can be converted into the SD by simply taking the square root of $s^2$
...
:::{.alert .alert-info}
##### <i class="fas fa-info-circle"></i> Standard Normal Distribution

- A variety of statistical techniques assume that the dependent variables under study are normally distributed in the population.
- If we assume a normal distribution then we can make a range of inferences about the probabilities of particular events or phenomenon.

</br>

```{r warning=FALSE}
# Define IQ distribution parameters
mean_IQ <- 100
sd_IQ <- 15

# Generate x-values (IQ scores ranging from 55 to 145)
...
# Create the plot
ggplot(df, aes(x, y)) +
  geom_line(color = "blue", size = 1.2) +  # Plot the normal curve
  geom_area(aes(y = y), fill = "skyblue", alpha = 0.5) +  # Shade the area under the curve
  geom_vline(xintercept = mean_IQ, color = "red", linetype = "dashed", size = 1.2) +  # Mean line
  annotate("text", x = mean_IQ, y = max(y) * 0.9, label = "Mean IQ = 100", color = "red", size = 5, vjust = -1) +
  labs(title = "IQ Score Distribution (N(100, 15))",
       x = "IQ Score",
       y = "Density") +
...
:::{.alert .alert-info}
##### <i class="fas fa-info-circle"></i> z-scores

- The standard normal distribution has a mean of 0 and a standard deviation of 1.
- We can convert any distribution into a standard normal distribution by applying the z-score formula, which converts each score into **standard deviation units.**
...
:::{.alert .alert-info}
##### <i class="fas fa-info-circle"></i> Using z-tables

- Consider an intelligence test which has a mean of 100 and a standard deviation of 15 in the general population.
- From this information we can work out how much of the area under the normal curve is above 1 standard deviations from the mean.
- That is, we can work out the probability of obtaining a score of 115 or more (+1 standard deviation) on this intelligence test.
- Given that z-scores represent standard deviation units we want to find the area above z-= 1.
- To answer this we need to consult the z-table.
- From Appendix z-you need to find the row corresponding to z-= 1 and locate the ‘smaller portion’ column, as we are interested in the area above that score.
- You should find that the smaller portion equals 0.1587.
- So, the probability that a person drawn randomly from the population will score 115 or above on this intelligence test is 0.1587 or approximately 16%.
:::

### LO6

:::{.alert .alert-info}
##### <i class="fas fa-info-circle"></i> Setting Probable Limits on z

- Setting limits on an observation provides researchers with a way of stating the confidence of their results.
- That is, having set limits on a score we can specify with a certain degree of confidence that the score will lie between two values.
- Usually researchers employ 95% confidence intervals so that they can say with 95% confidence that a score will lie between two values.
- We need to remove 5% of the area under the curve, which means we need to find the value that cuts off 2.5% at each end.
- Inspection of the z-tables (Appendix z) indicates that z-= ±1.96 corresponds to this value.
- This means that we can be 95% confident that a score on the intelligence test will fall between 1.96 standard deviations below and 1.96 standard deviations above the mean.
- So what does this mean for our intelligence test?
- This is simply a matter of calculating scores that correspond to 1.96 standard deviations from the mean.
- A way to calculate this is provided by Howell on page 74:
...
## Week 3: Sampling Distributions and Hypothesis Testing {.well .tabset .tabset-fade .tabset-pills}

### LO1

:::{.alert .alert-info}
#### <i class="fas fa-info-circle"></i> Sampling Error
...
#### <i class="fas fa-info-circle"></i>Sampling Distributions
...
:::{.alert .alert-info}
#### <i class="fas fa-info-circle"></i> Hypothesis Testing
...
:::{.alert .alert-info}
#### <i class="fas fa-info-circle"></i> Null Hypothesis ($H_0$)
...
:::{.alert .alert-info}
#### <i class="fas fa-info-circle"></i>Hypothesis Testing using the Normal Distribution
...
:::{.alert .alert-info}
#### <i class="fas fa-info-circle"></i> Type I and II errors
...
:::{.alert .alert-info}
#### <i class="fas fa-info-circle"></i> One and Two Tailed Tests
...
## Week 4: Chi-Square {.well .tabset .tabset-fade .tabset-pills}

### LO1

:::{.alert .alert-info}
#### <i class="fas fa-info-circle"></i>Chi-Square Goodness of Fit Test
...
## Week 5: t-tests {.well .tabset .tabset-fade .tabset-pills}
...
:::{.alert .alert-info}
#### <i class="fas fa-info-circle"></i> Overview of the t-test
...
:::{.alert .alert-warning}
#### <i class="fas fa-info-circle"></i> Reporting
...
\begin{align}
  ...
  &= \frac{15.31-12.19}{\sqrt{\frac{19.54}{10} + \frac{10.76}{10}}} \\
  &= \frac{3.12}{\sqrt{1.95+1.08}} \\
  &= \frac{3.12}{1.74} \\
  &= 1.79 \\
...
## Week 6: Correlation {.well .tabset .tabset-fade .tabset-pills}
...
#### Predicted/explained variance ($r^2$)
...
## Week 7: Regression {.well .tabset .tabset-fade .tabset-pills}
...
#### Anxiety vs Negative Mood Example
...
## Week 8: ANOVA {.well .tabset .tabset-fade .tabset-pills}
...
#### One-way ANOVA Summary Formulae
...
## Week 9: Post-hoc Tests {.well .tabset .tabset-fade .tabset-pills}



## Week 10: Factorial ANOVA {.well .tabset .tabset-fade .tabset-pills}
...
#### Two-way ANOVA Summary Table (Formulae)
...
## Formulae {.well .tabset .tabset-fade .tabset-pills}
